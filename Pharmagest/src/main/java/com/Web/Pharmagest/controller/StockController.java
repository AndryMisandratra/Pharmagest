...
public class StockController {
    ...
    // ================================================
    // PATCH /api/lots/{id}/dates
    // Mettre à jour datePeremption et/ou dateFabrication
    // PREPARATEUR ou PHARMACIEN
    // ================================================
    @PatchMapping("/api/lots/{id}/dates")
    @PreAuthorize("hasAnyAuthority('ROLE_PREPARATEUR','ROLE_PHARMACIEN')")
    public ResponseEntity<LotStockResponse> mettreAJourDatesLot(
            @PathVariable Long id,
            @Valid @RequestBody LotDateUpdateRequest request) {
        return ResponseEntity.ok(
                stockService.mettreAJourDatesLot(id, request));
    }
