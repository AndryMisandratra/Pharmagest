...
public class AuthController {
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginRequest request) {
        ...
        // 3. Si c'est la première connexion de la semaine pour n'importe quel utilisateur,
        //    envoie l'email d'alerte au pharmacien.
        var debutSemaine = LocalDate.now()
                .with(DayOfWeek.MONDAY)
                .atStartOfDay();
        boolean dejaConnexeCetteSemaine = utilisateurRepository
                .aAuMoinsUneConnexionDepuis(debutSemaine);

        if (!dejaConnexeCetteSemaine) {
