...
public class CommandeController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CommandeResponse> getById(
    ...
    // PUT /api/commandes/{id} — PREPARATEUR ou PHARMACIEN
    // Modifier une commande en BROUILLON
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority(" +
            "'ROLE_PREPARATEUR','ROLE_PHARMACIEN')")
    public ResponseEntity<CommandeResponse> modifier(
            @PathVariable Long id,
            @Valid @RequestBody CommandeRequest request,
            @AuthenticationPrincipal UserDetails userDetails) {
        return ResponseEntity.ok(
    ...
    // Les numéros de lot sont déjà générés-auto
    // Saisir : quantiteRecue, datePeremption, dateFabrication
